...
The Next.js <Link href="https://nextjs.org/docs/app/api-reference/components/image">Image component </Link> renders a single `<img>` element, which unfortunately cannot support adding a background image via CSS styles to another element.
...
**[@todo] -- add example code of how to use `getImageProps()` to render a background image on a div.**

This function could be used to get the image props and apply it via inline CSS styles added to the `styles` prop:
