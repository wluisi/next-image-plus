...
This guide will show you how to use the HTML `<picture>` element with a Next.js app. The HTML `<picture>` element allows developers to display different images for different devices or screen sizes. It enables **art direction**, where you serve entirely different images depending on the context, not just resized versions of the same image.
...
With the release of <Link href="https://nextjs.org/blog/next-14-1">v14.1.0</Link>, Next.js added a new function called `getImageProps()`.
...
The typical way to do this with the Image component, is setting `priority` to `true`. However, this has no effect when using `getImageProps()`, and does not add `<link rel="preload" as="image">` tag to the document's `<head>`.
...
If your image is flagged as the <Link href="https://web.dev/articles/lcp#what-elements-are-considered">Largest Contentful Paint (LCP)</Link>, preloading can improve performance and your core web vitals score.
