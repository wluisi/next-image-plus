...
While this can be used to generate the markup needed to support `<picture>`, preloading of images is not supported, as setting `priority` to `true` has no effect, and does not add `<link rel="preload" as="image">` tag to the document's `<head>`.
