---
title: "Picture Component (v1 draft)"
...
| modifyMediaQueries | `modifyMediaQueries={true}`          | Boolean | -      |
...
If this functionality causes any issues, it can be disabled with the `modifyMediaQueries` prop.
...
An optional prop to disable **next-image-plus** from modifying the media queries to remove overlap.
