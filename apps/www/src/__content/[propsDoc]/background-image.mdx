...
| ------------------------ | -------------------------------------------------------------- | -------------------------- | -------- |
| string                   | `id="examples__background-image"`                              | `string`                   | Required |
| React.ElementType        | `as="span"`                                                    | `React.ElementType`        | -        |
| boolean                  | `preload={false} // {false} \| {true}`                         | `boolean`                  | -        |
| BackgroundImageOptions[] | `images={[ ... ]}`                                             | `BackgroundImageOptions[]` | Required |
| string                   | `className="absolute inset-0 bg-cover bg-center bg-no-repeat"` | `string`                   | -        |
| React.CSSProperties      | `style={{ color: "red" }}`                                     | `React.CSSProperties`      | -        |
| React.ReactNode          | `<BackgroundImage>   <div>content</div> </BackgroundImage>`    | `React.ReactNode`          | -        |
| boolean                  | `normalizeMediaQueries={false} // {false} \| {true}`           | `boolean`                  | -        |
