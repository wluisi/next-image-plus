...
The **next-image-plus** `<Picture>` is a primitive React component that extends the HTML `<picture>` element to provide features for automatic image optimization,
...
The browser uses the first Source component with a media query that returns true. If none of the media queries match, the browser falls back to loading the image specified by the Image component.
