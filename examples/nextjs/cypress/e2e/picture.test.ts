...
describe("Fallback: Picture component", () => {
  beforeEach(() => {
    cy.viewport(430, 932);
  });

  it.each(routers)(
    "should make network request (preload) for fallback (mobile) picture image and not md or lg.",
    ({ router, slug }) => {
      // it(`[ ${router.router} ] should make network request (preload) for fallback (mobile) picture image and not md or lg.`, () => {
      interceptNextImage({
